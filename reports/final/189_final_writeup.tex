...
for the frequency vectors for each of the coefficients. Then, we
applied a sort of ``meta'' \texttt{k\_means} that treated each of the
output clusters from the previous step as an orthonormal basis. Again,
this is a crude algorithm (we really should be preserving more
information about the distances between clusters, instead of treating
them as equidistant), but we were under some heavy time constraints.
Finally, we applied \texttt{k\_means} to this, and after tuning some
hyperparameters, we were shocked to obtain moderately decent results.
...
% TODO: describe how we can predict further into the future with the Markov model
